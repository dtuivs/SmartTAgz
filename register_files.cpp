...
QString register_files::generateTimestamp()
{
...
bool register_files::checkFileExistence(QString filePath){
...
QString register_files::checkPersistenceFile(QString property)
{
    ...
    if(!checkFileExistence(persistenceFilePath)){
        qDebug() << "No Persistence file... Making one now...";
        if(persistenceFile.open(QIODevice::WriteOnly)){
            QTextStream writetofile(&persistenceFile);

            writetofile << "KEYPADHIDDEN: 0" << "\n";
            writetofile << "CURRENT_TICKET_LOCATION: " << settingsFolder << generateTimestamp()<<"_List.txt"<<"\n";
            writetofile << "TICKET_FOLDER_LOCATION: " << settingsFolder << "\n";
            writetofile << "DATABASE_FILE_LOCATION: " << settingsFolder << "upc_database.txt";
...
void register_files::updateDatabase(QString upc, QString description, QString price)
{
    ...
    while (!in.atEnd()) {
        ...
        if (line.startsWith(upc)) {
            ...
            if (parts.size() > 1
                && !pricePresent) { //check database for a price and whether input has new price
